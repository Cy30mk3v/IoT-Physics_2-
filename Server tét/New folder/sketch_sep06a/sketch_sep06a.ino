...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i=0; i<numNewMessages; i++) {
    ...
    if (text == "/getInfo") {
      ...
      double uv = getUvValue();
      double temperature = getTempValue();
      bot.sendMessage(chat_id,"Temperature: " + (String)temperature + " " + checkTempDanger(temperature), "");
      bot.sendMessage(chat_id,"Uv index: " + (String)uv + " " + checkUvDanger(uv), "");
      bot.sendMessage(chat_id,"Air Quality 2.5 PM: " + (String)dust + " " + checkDustDanger(dust), "");
      //ledState = LOW;
      //digitalWrite(ledPin, ledState);
    }
...
const char* checkTempDanger(double temp){
  if (temp < 26.0){
    return "Good";
  }
  if (temp < 33.0){
...
const char* checkUvDanger(double uv){
  if (uv < 0.067){
    return "Good";
  }
  if (uv < 0.15){
